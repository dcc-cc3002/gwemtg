...
class vinculo_estrechoTest extends munit.FunSuite {
  ...
  var geraltOfRivia: vinculo_estrechoCCC = _
  var yenneferOfVengerberg: vinculo_estrechoRCC = _
  var ciri: vinculo_estrechoSCC = _
  ...
  override def beforeEach(context: BeforeEach): Unit = {
    ...
    geraltOfRivia = new vinculo_estrechoCCC("Geralt of Rivia", "vinculo_estrecho", 5, 5)
    yenneferOfVengerberg = new vinculo_estrechoRCC("Yennefer of Vengerberg", "vinculo_estrecho", 3, 3)
    ciri = new vinculo_estrechoSCC("Ciri", "vinculo_estrecho", 4, 4)
    vesemir = new SiegeCombatCard("Vesemir", "vinculo_estrecho", 5, 5)
    dandelion = new SiegeCombatCard("Dandelion", "vinculo_estrecho", 3, 3)
    trissMerigold = new CloseCombatCard("Triss Merigold", "vinculo_estrecho", 5, 5)
    yen = new RangedCombatCard("Yen", "vinculo_estrecho", 6, 6)
    eskel = new CloseCombatCard("Eskel", "vinculo_estrecho", 3, 3)
    lambert = new SiegeCombatCard("Lambert", "vinculo_estrecho", 6, 6)
  ...
  test("check usage for player 1") {
    var board = new Board()
    var player = new Player("Hugo", 2, List(), List(geraltOfRivia, yenneferOfVengerberg, ciri, geraltOfRivia, yenneferOfVengerberg, ciri))
    var game = new Game(board, player, jugador1)

    assertEquals(game.boardPoints(player), 0)
    player.playCard(geraltOfRivia, game)
    assertEquals(game.boardPoints(player), 5)
    player.playCard(geraltOfRivia, game)
    assertEquals(game.boardPoints(player), 20)
    player.playCard(yenneferOfVengerberg, game)
    assertEquals(game.boardPoints(player), 23)
    player.playCard(yenneferOfVengerberg, game)
    assertEquals(game.boardPoints(player), 32)
    player.playCard(ciri, game)
    assertEquals(game.boardPoints(player), 36)
    player.playCard(ciri, game)
    assertEquals(game.boardPoints(player), 40)
  }
