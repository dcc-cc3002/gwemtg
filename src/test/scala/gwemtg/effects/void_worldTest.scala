...
import gwemtg.effects.{double_D3, void_world}

import munit.*

class void_worldTest extends munit.FunSuite {
  ...
  override def beforeEach(context: BeforeEach): Unit = {
    ...
    clearWeather = new void_world("Despejar", "Limpia todos los efectos de clima del campo de batalla.")
  ...
  test("check if the effect clears the board of modifiers"){
    var board = new Board()
    var player = new Player("Hugo", 2, List(), List(troop1,ranged1,catapult1,clearWeather))
    ...
    player.playCard(clearWeather,game)
    assertEquals(game.boardPoints(player),3)
  }
