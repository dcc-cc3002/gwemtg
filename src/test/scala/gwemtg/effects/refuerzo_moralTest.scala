...
class refuerzo_moralTest extends munit.FunSuite {
  ...
  var geraltOfRivia: refuerzo_moralCCC = _
  var yenneferOfVengerberg: refuerzo_moralRCC = _
  var ciri: refuerzo_moralSCC = _
  ...
  override def beforeEach(context: BeforeEach): Unit = {
    ...
    geraltOfRivia = new refuerzo_moralCCC("Geralt of Rivia", "refuerzo_moral", 5, 5)
    yenneferOfVengerberg = new refuerzo_moralRCC("Yennefer of Vengerberg", "refuerzo_moral", 3, 3)
    ciri = new refuerzo_moralSCC("Ciri", "refuerzo_moral", 4, 4)
    vesemir = new SiegeCombatCard("Vesemir", "refuerzo_moral", 5, 5)
    dandelion = new SiegeCombatCard("Dandelion", "refuerzo_moral", 3, 3)
    trissMerigold = new CloseCombatCard("Triss Merigold", "refuerzo_moral", 5, 5)
    yen = new RangedCombatCard("Yen", "refuerzo_moral", 6, 6)
    eskel = new CloseCombatCard("Eskel", "refuerzo_moral", 3, 3)
    lambert = new SiegeCombatCard("Lambert", "refuerzo_moral", 6, 6)
    zoltanChivay = new CloseCombatCard("Zoltan Chivay", "refuerzo_moral", 4, 4)
  ...
  test("check usage for player 1") {
    ...
    player.playCard(geraltOfRivia, game)
    assertEquals(game.boardPoints(player), 12)
    player.playCard(yenneferOfVengerberg, game)
    assertEquals(game.boardPoints(player), 15)
    player.playCard(yenneferOfVengerberg, game)
    assertEquals(game.boardPoints(player), 20)
    player.playCard(ciri, game)
    assertEquals(game.boardPoints(player), 24)
    player.playCard(ciri, game)
    assertEquals(game.boardPoints(player), 30)
  }
